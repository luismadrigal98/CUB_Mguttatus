...
summarise_recombination_by_expression <- function(rec_table, gene_meta,
  ...
  data.table::setattr(out, "per_gene", d[, .(Gene_name, recombRate, Exp_cat)])
  out[]
}


plot_recombination_by_expression <- function(rec_gene_table, summary_table) {
  #' Crossover rate across the expression range
  #'
  #' Two panels of the same relationship, because a single one can mislead in
  #' either direction. Panel A shows the whole per-gene distribution on an axis
  #' anchored at zero: crossover rate spans the full range of the genome inside
  #' every expression category, so whatever association exists is negligible
  #' beside the scatter. Panel B is the category means on an expanded axis, so
  #' the small real structure — a rise between the two lowest categories, flat
  #' thereafter — is not hidden either. Plotting only B would magnify a 14%
  #' range into an apparent trend; plotting only A would hide it.

  suppressPackageStartupMessages({require(ggplot2); require(patchwork); require(data.table)})
  d  <- data.table::as.data.table(rec_gene_table)
  st <- data.table::as.data.table(summary_table)
  d  <- d[Exp_cat %in% st$Exp_cat]
  brk <- seq(0, max(st$Exp_cat), 0.4)

  pA <- ggplot(d, aes(x = Exp_cat, y = recombRate, group = Exp_cat)) +
    geom_boxplot(varwidth = TRUE, outlier.size = 0.25, outlier.alpha = 0.15,
                 fill = "#D9D9D9", colour = "#4D4D4D", linewidth = 0.3) +
    geom_line(data = st, aes(x = Exp_cat, y = rate_mean, group = 1),
              colour = "#B2182B", linewidth = 0.7) +
    geom_point(data = st, aes(x = Exp_cat, y = rate_mean, group = 1),
               colour = "#B2182B", size = 1.8) +
  ...
  pB <- ggplot(st, aes(x = Exp_cat, y = rate_mean)) +
