...
void Trace::initializeROCTrace(unsigned samples, unsigned num_genes, unsigned numMutationCategories,
	...
	std::vector<unsigned> init_mix_assign,bool estimateSynthesisRate)
{
	// FIX (LJMR): pass numObservedPhiSets explicitly; previously estimateSynthesisRate (bool)
	// was being passed in that position due to a missing argument, which caused the
	// SynthesisOffset and ObservedSynthesisNoise traces to be initialized with size 1
	// instead of the actual number of phi groupings, leading to a segfault when
	// withPhi=TRUE and numObservedPhiSets > 1.
	initializeSharedTraces(samples, num_genes, numSelectionCategories, numMixtures, _categories, maxGrouping,init_phi,init_mix_assign,numObservedPhiSets,estimateSynthesisRate);
